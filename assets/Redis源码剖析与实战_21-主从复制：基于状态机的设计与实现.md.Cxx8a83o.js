import{_ as n,H as a,f as p,i as e}from"./chunks/framework.BH2BK_3i.js";const h=JSON.parse('{"title":"21 | 主从复制：基于状态机的设计与实现","description":"","frontmatter":{},"headers":[{"level":2,"title":"主从复制的四大阶段","slug":"主从复制的四大阶段","link":"#主从复制的四大阶段","children":[]},{"level":2,"title":"基于状态机的主从复制实现","slug":"基于状态机的主从复制实现","link":"#基于状态机的主从复制实现","children":[{"level":3,"title":"初始化阶段","slug":"初始化阶段","link":"#初始化阶段","children":[]},{"level":3,"title":"建立连接阶段","slug":"建立连接阶段","link":"#建立连接阶段","children":[]},{"level":3,"title":"主从握手阶段","slug":"主从握手阶段","link":"#主从握手阶段","children":[]},{"level":3,"title":"复制类型判断与执行阶段","slug":"复制类型判断与执行阶段","link":"#复制类型判断与执行阶段","children":[]}]},{"level":2,"title":"小结","slug":"小结","link":"#小结","children":[]},{"level":2,"title":"每课一问","slug":"每课一问","link":"#每课一问","children":[]}],"relativePath":"Redis源码剖析与实战/21-主从复制：基于状态机的设计与实现.md","filePath":"Redis源码剖析与实战/21-主从复制：基于状态机的设计与实现.md","lastUpdated":1779816274000}'),t={name:"Redis源码剖析与实战/21-主从复制：基于状态机的设计与实现.md"};function i(l,s,r,c,o,E){return a(),p("div",null,[...s[0]||(s[0]=[e(`<h1 id="_21-主从复制-基于状态机的设计与实现" tabindex="-1">21 | 主从复制：基于状态机的设计与实现 <a class="header-anchor" href="#_21-主从复制-基于状态机的设计与实现" aria-label="Permalink to &quot;21 | 主从复制：基于状态机的设计与实现&quot;">​</a></h1><p>你好，我是蒋德钧。这节课，我想跟你聊聊Redis是如何基于状态机的设计思路，来实现主从复制的。</p><p>主从复制技术我们应该都比较熟悉，因为在使用Redis或MySQL数据库时，我们经常会使用主从复制来实现主从节点间的数据同步，以此提升服务的高可用性。</p><p>从原理上来说，Redis的主从复制主要包括了 <strong>全量复制、增量复制和长连接同步</strong> 三种情况。全量复制传输RDB文件，增量复制传输主从断连期间的命令，而长连接同步则是把主节点正常收到的请求传输给从节点。</p><p>这三种情况看似简单，但是在实现的时候，我们通常都需要考虑主从连接建立、主从握手和验证、复制情况判断和数据传输等多种不同状态下的逻辑处理。</p><p>那么， <strong>如何才能高效地实现主从复制呢？</strong></p><p>实际上，Redis是采用了 <strong>基于状态机</strong> 的设计思想，来清晰地实现不同状态及状态间的跳转。而在我们实现网络功能的时候，这种设计和实现方法其实非常重要，它可以避免我们在处理不同状态时的逻辑冲突或遗漏。所以今天这节课，我就来给你介绍下如何基于状态机实现主从复制。</p><p>不过这里我也要说明一点，因为主从复制的状态比较多，如果一下子就学习每个状态细节，我们其实会很容易混淆不同状态的区别和转换关系。所以在今天的课程中，我会先给你介绍下复制整体过程的四个阶段，然后，我们再来逐一学习每个阶段中的状态与变化。</p><h2 id="主从复制的四大阶段" tabindex="-1">主从复制的四大阶段 <a class="header-anchor" href="#主从复制的四大阶段" aria-label="Permalink to &quot;主从复制的四大阶段&quot;">​</a></h2><p>首先，我们可以根据主从复制时的关键事件，把整个复制过程分成四个阶段，分别是初始化、建立连接、主从握手、复制类型判断与执行。下面，我们就来依次了解下每个阶段的主要工作。</p><p><strong>1.初始化阶段</strong></p><p>当我们把一个Redis实例A设置为另一个实例B的从库时，实例A会完成初始化操作，主要是获得了主库的IP和端口号。而这个初始化过程，我们可以用三种方式来设置。</p><ul><li>方式一：在实例A上执行replicaof masterip masterport的主从复制命令，指明实例B的IP（masterip）和端口号（masterport）。</li><li>方式二：在实例A的配置文件中设置replicaof masterip masterport，实例A可以通过解析文件获得主库IP和端口号。</li><li>方式三：在实例A启动时，设置启动参数–replicaof [masterip] [masterport]。实例A解析启动参数，就能获得主库的IP和端口号。</li></ul><p><strong>2.建立连接阶段</strong></p><p>接下来，一旦实例A获得了主库IP和端口号，该实例就会尝试和主库建立TCP网络连接，并且会在建立好的网络连接上，监听是否有主库发送的命令。</p><p><strong>3.主从握手阶段</strong></p><p>当实例A和主库建立好连接之后，实例A就开始和主库进行握手。简单来说，握手过程就是主从库间相互发送PING-PONG消息，同时从库根据配置信息向主库进行验证。最后，从库把自己的IP、端口号，以及对无盘复制和PSYNC 2协议的支持情况发给主库。</p><p>那么，和前两个阶段相比，主从握手阶段要执行的操作会比较多，涉及的状态也比较多，所以我们需要先掌握这个阶段要完成的操作，一会儿我就来给你具体介绍。</p><p><strong>4.复制类型判断与执行阶段</strong></p><p>这样，等到主从库之间的握手完成后，从库就会给主库发送PSYNC命令。紧接着，主库会根据从库发送的命令参数作出相应的三种回复，分别是 <strong>执行全量复制、执行增量复制、发生错误</strong>。最后，从库在收到上述回复后，就会根据回复的复制类型，开始执行具体的复制操作。</p><p>下图展示了主从复制的整体过程及四个阶段，你可以看下。</p><p><a class="image-link" href="https://raw.githubusercontent.com/qiangshuifish/note-website/note-website/Redis%E6%BA%90%E7%A0%81%E5%89%96%E6%9E%90%E4%B8%8E%E5%AE%9E%E6%88%98/images/420285/c0e917700f6146712bf9a74830d9d4c4.jpg" target="_blank" rel="noopener" title="点击查看原图"><img src="https://raw.githubusercontent.com/qiangshuifish/note-website/note-website/Redis%E6%BA%90%E7%A0%81%E5%89%96%E6%9E%90%E4%B8%8E%E5%AE%9E%E6%88%98/images/420285/c0e917700f6146712bf9a74830d9d4c4.jpg" alt="图片"></a></p><p>好，了解了主从复制的主要阶段后，接下来，我们就具体学习下Redis是如何使用不同的状态及转换，来让从库完成和主库的数据复制操作的。</p><h2 id="基于状态机的主从复制实现" tabindex="-1">基于状态机的主从复制实现 <a class="header-anchor" href="#基于状态机的主从复制实现" aria-label="Permalink to &quot;基于状态机的主从复制实现&quot;">​</a></h2><p>首先你要知道，基于状态机实现主从复制的好处，就是当你在开发程序时，只需要考虑清楚在不同状态下具体要执行的操作，以及状态之间的跳转条件就行了。所以，Redis源码中采用的基于状态机跳转的设计思路和主从复制的实现，就是很值得你学习的一点。</p><p><strong>那么，主从复制中的状态机具体对应的是什么呢？</strong> 这就和Redis实例的数据结构有关了。</p><p>每一个Redis实例在代码中都对应一个 <strong>redisServer结构体</strong>，这个结构体包含了和Redis实例相关的各种配置，比如实例的RDB、AOF配置、主从复制配置、切片集群配置等。然后，与主从复制状态机相关的变量是 <strong>repl_state</strong>，Redis在进行主从复制时，从库就是根据这个变量值的变化，来实现不同阶段的执行和跳转。下面代码显示了redisServer结构体中从库进行复制相关的变量，你可以看下。</p><div class="language-plain vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">plain</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>struct redisServer {</span></span>
...
<span class="line"><span>}</span></span></code></pre></div><p>所以，当从库实例的状态变为REPL_STATE_CONNECTING时，建立连接的阶段就完成了。这个初始化阶段和建立连接阶段的状态机变迁如下图所示，你可以参考下。</p><p><a class="image-link" href="https://raw.githubusercontent.com/qiangshuifish/note-website/note-website/Redis%E6%BA%90%E7%A0%81%E5%89%96%E6%9E%90%E4%B8%8E%E5%AE%9E%E6%88%98/images/420285/dd6176abeb3ba492f15a93bd0b4a84aa.jpg" target="_blank" rel="noopener" title="点击查看原图"><img src="https://raw.githubusercontent.com/qiangshuifish/note-website/note-website/Redis%E6%BA%90%E7%A0%81%E5%89%96%E6%9E%90%E4%B8%8E%E5%AE%9E%E6%88%98/images/420285/dd6176abeb3ba492f15a93bd0b4a84aa.jpg" alt="图片"></a></p><h3 id="主从握手阶段" tabindex="-1"><strong>主从握手阶段</strong> <a class="header-anchor" href="#主从握手阶段" aria-label="Permalink to &quot;**主从握手阶段**&quot;">​</a></h3><p>接下来，当主从库建立网络连接后，从库实例其实并没有立即开始进行数据同步，而是会先和主库之间进行握手通信。</p><p>握手通信的目的，主要包括从库和主库进行验证，以及从库将自身的IP和端口号发给主库。如我前面所说，这个阶段涉及的状态变迁会比较多，不过其变迁的逻辑实际上是比较清晰的。</p><p>首先，在建立连接阶段的最后，从库实例的状态机处于 <strong>REPL_STATE_CONNECTING</strong> 状态。一旦主库和从库的连接建立后，从库实例的syncWithMaster函数就会被回调。在这个函数中，如果从库实例的状态是REPL_STATE_CONNECTING，那么实例会发送PING消息给主库，并将状态机置为 <strong>REPL_STATE_RECEIVE_PONG</strong>。</p><p>当从库收到主库返回的PONG消息后，接下来，从库会依次给主库发送验证信息、端口号、IP、对RDB文件和无盘复制的支持情况。每一次的握手通信发送消息时，都会对应从库的一组状态变迁。比如，当从库要给主库发送验证信息前，会将自身状态机置为REPL_STATE_SEND_AUTH，然后，从库给主库发送实际的验证信息。验证信息发送完成后，从库状态机会变迁为REPL_STATE_RECEIVE_AUTH，并开始读取主库返回验证结果信息。</p><p>这样一来，当从库对端口号、IP，以及对RDB文件和无盘复制的支持情况进行握手时，也就是在SEND和RECEIVE两种状态间变迁。为了便于你掌握这些状态的变迁，这里我放了一张图，其中显示了从初始化阶段到主从握手阶段的各状态变化，你可以参考下。</p><p><a class="image-link" href="https://raw.githubusercontent.com/qiangshuifish/note-website/note-website/Redis%E6%BA%90%E7%A0%81%E5%89%96%E6%9E%90%E4%B8%8E%E5%AE%9E%E6%88%98/images/420285/c2946565d547bd52063ff1a79ec426cf.jpg" target="_blank" rel="noopener" title="点击查看原图"><img src="https://raw.githubusercontent.com/qiangshuifish/note-website/note-website/Redis%E6%BA%90%E7%A0%81%E5%89%96%E6%9E%90%E4%B8%8E%E5%AE%9E%E6%88%98/images/420285/c2946565d547bd52063ff1a79ec426cf.jpg" alt="图片"></a></p><h3 id="复制类型判断与执行阶段" tabindex="-1"><strong>复制类型判断与执行阶段</strong> <a class="header-anchor" href="#复制类型判断与执行阶段" aria-label="Permalink to &quot;**复制类型判断与执行阶段**&quot;">​</a></h3><p>当从库和主库完成握手后，从库会读取主库返回的CAPA消息响应，此时，状态机为 <strong>REPL_STATE_RECEIVE_CAPA</strong>。紧接着，从库的状态变迁为 <strong>REPL_STATE_SEND_PSYNC</strong>，表明要开始向主库发送PSYNC命令，开始实际的数据同步。</p><p>此时，从库会调用slaveTryPartialResynchronization函数，向主库发送PSYNC命令，并且状态机的状态会置为 <strong>REPL_STATE_RECEIVE_PSYNC</strong>。下面的代码显示了这三个状态的变迁：</p><div class="language-plain vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">plain</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>    /* 从库状态机进入REPL_STATE_RECEIVE_CAPA. */</span></span>
...
<span class="line"><span>    server.repl_state = REPL_STATE_TRANSFER;</span></span></code></pre></div><p>好了，到这里，我们就学习了从库在复制类型判断和执行阶段的状态机变迁。我把主从复制各阶段的状态变迁整合在一起，画了下面这张图，以便你更好地掌握。</p><p><a class="image-link" href="https://raw.githubusercontent.com/qiangshuifish/note-website/note-website/Redis%E6%BA%90%E7%A0%81%E5%89%96%E6%9E%90%E4%B8%8E%E5%AE%9E%E6%88%98/images/420285/f6e25eb125f0d70694d92597dca3e197.jpg" target="_blank" rel="noopener" title="点击查看原图"><img src="https://raw.githubusercontent.com/qiangshuifish/note-website/note-website/Redis%E6%BA%90%E7%A0%81%E5%89%96%E6%9E%90%E4%B8%8E%E5%AE%9E%E6%88%98/images/420285/f6e25eb125f0d70694d92597dca3e197.jpg" alt="图片"></a></p><h2 id="小结" tabindex="-1">小结 <a class="header-anchor" href="#小结" aria-label="Permalink to &quot;小结&quot;">​</a></h2><p>主从复制是Redis、MySQL等数据库或存储系统，用来实现高可用性的方法。要实现主从复制，则需要应对整个过程中Redis在不同状态下的各种处理逻辑，因此，如何正确实现主从复制，并且不遗漏可能的状态，是我们在实际开发中需要面对的问题。</p><p>这节课我们学习了Redis主从复制的设计思想与实现方法。Redis采用了 <strong>状态机驱动</strong> 的方法，为从库实例设置状态变量。在整个复制过程中，代码逻辑会根据从库状态机的变迁，处理不同状态下的情况。</p><p>为了便于你掌握主从复制的实现，我将整个过程分解成四个阶段：初始化、建立连接、主从握手、复制类型判断与执行。在每个阶段中，从库的状态会不断变化，完成和主库建立网络连接、交换配置信息、发送同步命令，并根据主库对同步请求的返回结果，执行全量同步或增量同步。</p><p>状态机驱动的设计方法是一种通用的设计方法，在涉及网络通信的场景中应用广泛。Redis对主从复制的实现为我们提供了良好的参考示例，当你需要自行设计和实现网络功能时，就可以把状态机驱动的方法使用起来。</p><h2 id="每课一问" tabindex="-1">每课一问 <a class="header-anchor" href="#每课一问" aria-label="Permalink to &quot;每课一问&quot;">​</a></h2><p>这节课我们介绍的状态机是当实例为从库时会使用的。那么，当一个实例是主库时，为什么不需要使用一个状态机来实现主库在主从复制时的流程流转呢？</p>`,75)])])}const g=n(t,[["render",i]]);export{h as __pageData,g as default};
