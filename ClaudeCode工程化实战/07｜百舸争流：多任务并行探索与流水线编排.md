...
![](images/945070/18dcaeeb751005b678e0b9363900d9fb.jpg)
...
![](images/945070/936b91a3466d6f5ff16964a6e88bb58c.jpg)
...
- db-explorer发现 users 表有 role 字段，但 orders 表里也有 user\_role 冗余字段 。
...
![](images/945070/29158b9e4c263abd13a6cac88500961f.jpg)
...
![](images/945070/ec07447b1c6fcf37cd15dd0f9670c562.jpg)
...
到目前为止，我们看到了流水线的骨架——四个阶段能够各司其职。但在实际使用中，流水线最容易出问题的不是每个阶段本身，而是 **阶段之间的信息传递**。你可能已经意识到了，这里的所谓交接，就是我们在 [第 4 讲](https://time.geekbang.org/column/article/943942)“量体裁衣”中所介绍的 **Handoffs模式** 的应用。
...
![](images/945070/28yy3c8646f021bfe863791b5002b9da.jpg)
...
这是因为Verifier 的失败信息是 **新的证据**，应该反馈给 Analyzer 而不是 Fixer。Fixer 只负责执行，不负责判断方向。
...
![](images/945070/4217eb94383a247c592ca6141d9186aa.jpg)
...
1. 你的项目中有没有适合并行探索的场景？比如需要同时理解多个模块的情况？

2. 除了 bug 修复，还有哪些任务适合用流水线模式？比如代码迁移、功能开发等。

3. 如果一个任务既有并行部分又有串行部分，你会怎么设计子代理的组合？

4. 为以下流水线设计交接契约——需求文档 → 技术方案 → 代码实现 → 测试验证。每个阶段的 Handoff 应该包含哪些字段？

5. 假设你的 bug-fixer 修复后，bug-verifier 跑测试发现原来的 bug 没修好，但引入了一个新的测试失败。你作为编排者应该怎么操作？写出具体的指令。
...
1. 进入 `projects/04-parallel-explore/` 目录

2. 运行 `同时让三个 explorer 探索各自模块，汇总成整体架构理解`
...
1. 进入 `projects/05-bugfix-pipeline/` 目录

2. 用流水线方式修复其中一个 bug

3. 在中间阶段试着介入，看看如何影响后续阶段
...
2. 判断：如果你是 Analyzer，这些信息够不够你开始分析？缺什么？

3. 修改 Locator 的 prompt，让 Handoff 部分更完整，重新跑一次，对比差异。


**练习四：混合模式实操**

1. 用 Fan-out → Fan-in 模式理解项目：同时让三个 explorer 探索，然后综合分析。

2. 基于综合分析中发现的问题，切换到流水线模式修复其中一个。

3. 思考：哪些步骤你做了人工介入？哪些步骤你让它自动继续了？为什么？


按照原定计划下一讲我们将进入新的主题： **Skills 技能系统**。不过，Claude在2026年2月6日最新推出的“ [团队协作](https://code.claude.com/docs/en/agent-teams)”（Agent Teams）功能很明显更适合整合进入SubAgent专题，因此佳哥正在积极创作相关内容，期待周五和大家见面。
