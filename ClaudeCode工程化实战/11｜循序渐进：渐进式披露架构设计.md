...
前两讲我们学习了 SKILL.md 的基本结构（ [第 9 讲](https://time.geekbang.org/column/article/945937)）和任务型 Skills 的设计方法（ [第 10 讲](https://time.geekbang.org/column/article/946297)），也理解了 Skills 在 Agent 生态中的核心定位—— **可操作知识**。但真实场景中，专业能力往往很复杂，比如一个财务分析 Skill，它需要包含财务指标计算公式、不同行业的基准数据、各种报表模板、分析脚本以及案例示例。
...
![](images/946847/2cc5dddcf2df981641366559e36eb847.jpg)
...
![](images/946847/c9d4312a913271ff4bbd178704bc7b47.jpg)

这就是渐进式披露的经济学基础：

![](images/946847/b980737e59bdcab48bbbc73d4e875601.jpg)

**渐进式披露的本质**：以最小的 token 投入获得最高的任务完成质量——这就是知识的 **投资回报率（Knowledge ROI）**。

![](images/946847/b7e0c64467273f12cb8b33134a39bfd7.jpg)

Token 节省效果方面。让我们用数字说话。假设一个复杂的 Skill 是这样的：

![](images/946847/byy10d190b3992924d57950yyeedf180.jpg)

不同场景下的 Token 消耗（大概估算哈）。

![](images/946847/4f8e9d5756915fcc86c8d37413615150.jpg)
...
![](images/946847/1d795791cba353404e8dc9b203d74054.jpg)
...
![图片](images/946847/c083f300c03c69e009f5b763bbba0f57.png)
...
![图片](images/946847/f18c16d1b44dd2be9c023yybbb613e53.png)

如图所示，此时，Claude只读取了 .claude/skills/financial-analyzing/reference/profitability.md这一个文件的83行。

![图片](images/946847/76f7f2dc7062747a865470c2a136ddd1.png)
...
4. 需要报告格式，加载 templates/analysis\_report.md

5. 需要计算 ，执行 scripts/calculate\_ratios.py


**Token 消耗**：全部资源都用到了。

![图片](images/946847/17e4cce4cd7d4761367993929021c9e8.png)

两个场景的关键对比如下。

![](images/946847/4e0c43f051cfeb9a3a01f6f8da0a10dc.jpg)
...
这和 [第 6 讲](https://time.geekbang.org/column/article/944525) 子代理流水线中的“交接契约”是同一个工程思想： **下游消费者需要知道上游提供什么，而不只是知道上游在哪里。**
...
脚本不仅能做计算，还能 **生成可视化结果**。Claude官方提供了一种强大的模式：Skill 中的脚本生成交互式 HTML 文件，在浏览器中打开。
...
![](images/946847/2faf0e3dc7aba13d43af28d2742e58fd.jpg)
...
![](images/946847/fc776b44b271e47f3d34a01d46eb5d37.jpg)

拆分决策树和目录结构不是随意设计的——它们映射到 **企业知识管理的经典范式。**

![](images/946847/3b0a7c0b162d48c954d259ded2cfc562.jpg)
...
![](images/946847/56811690458e1b73f1db3fcfefe9be6e.jpg)
...
![](images/946847/e6dd57ea37aa8f87af69ecce9e802526.jpg)

第三层关系是 **Tools 反哺 Skills。** [第 10 讲](https://time.geekbang.org/column/article/946297?utm_source=pinpaizhuanqu&utm_medium=geektime&utm_campaign=guanwang&utm_term=guanwang&utm_content=0511) 介绍的 `!` command\`\` 语法展示了反向关系—— **Tools 的输出反哺 Skills 的上下文**：
...
![](images/946847/b358d3765aaa8ffd0d0b980e426d371e.jpg)![](images/946847/23fb10ffe0d06f749436d7bdb19379c2.jpg)
...
**SKILL.md 是路由器，通过Quick Reference 表格用最少 token 完成路由判断。引用文件的“契约式”写法，即契约式引用会** 告诉 Claude 何时加载、加载什么、得到什么。这里有一个 **500 行法则**，超过就该重构——把参考、模板、示例移到辅助文件。通过 **脚本封装** 来封装确定性逻辑，Claude 执行但不需要理解脚本中的内容。

课程学到这里，我们已经掌握了两种上下文管理策略，分别是 **Skill的渐进式披露（按需加载）+ 子代理隔离（纯净上下文）**。它们互相补充，解决不同的问题。我最后给出 **两种上下文管理策略表的对比。**

![](images/946847/ec5c0d563dac7161c7026eb14f559221.jpg)
...
1. 如果你的 Skill 有 20 个参考文件，你会如何组织，才能让 Claude 快速找到需要的内容？

2. 什么样的内容应该放在 SKILL.md 主文件，什么样的应该放在引用文件？

3. 渐进式披露和我们之前学的子代理“隔离上下文”有什么相似之处？

4. 你有一个 Skill 的 SKILL.md 现在是 800 行，包含：100 行核心指令、200 行 API 规范、300 行代码示例、200 行错误处理模板。用拆分决策树重构它——画出重构后的目录结构，写出 SKILL.md 中对每个文件的“契约式引用”。

5. 设计一个“可视化输出”类型的 Skill：选择你项目中的一个分析需求（测试覆盖率/依赖关系/API 调用链），设计 SKILL.md + scripts/ 的目录结构，说明 Claude 只需要知道什么（最少 token），脚本负责什么（最多逻辑）。

6. 用“认知经济学”分析你项目中一个现有 Skill（或设计一个新的）：画出 token 投入 vs 任务完成质量的曲线。哪些知识是关键知识（高边际收益）？哪些是冗余知识（接近零收益）？如何通过渐进式披露把 ROI 最大化？

7. 分析 Skills × Tools 的三层关系在你项目中的应用：你的 Skill 需要约束哪些 Tools？有哪些确定性逻辑适合封装为脚本（预编译知识）？有哪些实时数据适合用 `!` command\`\` 预加载（Tools 反哺）？你可以自己设计出你的 Skills × Tools 协作图。
